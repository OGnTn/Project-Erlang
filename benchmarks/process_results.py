...
def plot_speedup_errorbars(speedups: dict[int, list[float]]):
    ...
    errors_down = [np.median(speedups[i]) - np.quantile(speedups[i], 0.25) for i in speedups]
    errors_up = [np.quantile(speedups[i], 0.75) - np.median(speedups[i]) for i in speedups]
